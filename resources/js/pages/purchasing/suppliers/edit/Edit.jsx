...
import { replaceSupplier } from "@/actions";
import api, { routes } from "@/api";

let cancelRequest;

class PurchasingSuppliersEdit extends Component {
  constructor(props) {
    super(props);

    const supplier = props.suppliers.filter(
      supplier => supplier.id == props.match.params.id
    )[0];
    if (!supplier) {
      return props.history.replace("/purchasing/suppliers");
    }
    this.state = {
      id: supplier.id,
      name: supplier.name,
  ...
  handleSave() {
    const { id, name, address, phone, email, saving } = this.state;
    if (saving) {
      return;
    }
    return this.setState({ saving: true, errors: {} }, async () => {
      try {
        ...
        this.props.replaceSupplier(supplier);

        return this.setState({ dirty: false }, () =>
          this.props.history.push("/purchasing/suppliers?_flow=edit-success")
        );
      } catch (error) {
        if (!isCancel(error)) {
  ...
  render() {
    const { name, address, phone, email, dirty, saving, errors } = this.state;
...
const mapDispatchToProps = { replaceSupplier };

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(PurchasingSuppliersEdit);
